<div id="editPersonalModal" class="fixed inset-0 z-[100] hidden overflow-y-auto">
    <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0">
        ...
        <div class="inline-block align-bottom bg-white rounded-md text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full relative z-10">
            ...
            <form id="editPersonalForm" action="{{ route('admin.profile.update.personal') }}" method="POST">
                @csrf
                @method('PUT')
                <div class="px-6 py-6 space-y-4">
                    <div>
                        <label for="full_name" class="block text-xs font-bold text-gray-400 uppercase mb-1">Full Name</label>
                        <input type="text" name="full_name" id="full_name" value="{{ $admin->full_name }}" 
                    ...
                    <div>
                        ...
                        <input type="text" name="ic_number" id="ic_number" value="{{ $admin->ic_number }}" 
                            class="w-full px-4 py-2.5 rounded-lg border border-gray-200 focus:border-blue-500 focus:ring-2 focus:ring-blue-200 transition-all text-sm outline-none" required>
                    </div>
