...
<head>
    ...
    <title>Vaccination Record - {{ $immunisation->vaccine_name }}</title>
...
<body class="bg-gray-100 py-10">
    <div class="max-w-4xl mx-auto bg-white shadow-lg rounded-xl overflow-hidden print-container">
        ...
        <div class="bg-blue-600 p-8 text-white flex justify-between items-center">
            <div>
                <h1 class="text-3xl font-bold">Vaccination Record</h1>
        ...
        <div class="p-8">
            ...
            <div class="grid grid-cols-2 gap-8 mb-10 pb-8 border-b border-gray-200">
                ...
                <div class="text-right">
                    ...
                    <span class="inline-flex items-center px-3 py-1 rounded-full text-sm font-semibold bg-blue-100 text-blue-700">
                        {{ $immunisation->verification_status ?? 'Patient Reported' }}
                    </span>
                </div>
            </div>
            ...
            <div class="mb-10">
                ...
                <div class="bg-gray-50 rounded-xl p-6 grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        <p class="text-sm text-gray-500">Vaccine Name</p>
                        <p class="font-semibold text-gray-900 text-lg">{{ $immunisation->vaccine_name }}</p>
                    </div>
                    <div>
                        <p class="text-sm text-gray-500">Vaccination Date</p>
                        <p class="font-semibold text-gray-900">{{ \Carbon\Carbon::parse($immunisation->vaccination_date)->format('F d, Y') }}</p>
                    </div>
                    <div>
                        <p class="text-sm text-gray-500">Dose Details</p>
                        <p class="font-semibold text-gray-900">{{ $immunisation->dose_details ?? 'Not specified' }}</p>
                    </div>
                    <div>
                        <p class="text-sm text-gray-500">Administered By</p>
                        <p class="font-semibold text-gray-900">{{ $immunisation->administered_by ?? 'Not specified' }}</p>
                    </div>
                    <div>
                        <p class="text-sm text-gray-500">Lot Number</p>
                        <p class="font-semibold text-gray-900">{{ $immunisation->vaccine_lot_number ?? 'Not specified' }}</p>
                    </div>
                </div>
            </div>
