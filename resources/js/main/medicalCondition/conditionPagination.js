/**
 * Pagination and Search functionality for Medical Conditions
 */
document.addEventListener('DOMContentLoaded', () => {
    ...
    const conditionCards = Array.from(document.querySelectorAll('[data-severity][data-status]'));
    
    /**
     * Get all visible cards based on current filters and search
     */
    function getVisibleCards() {
        return conditionCards.filter(card => {
    ...
    function updateCounter() {
        if (conditionsCount) {
            ...
            if (total === 0) {
                conditionsCount.innerHTML = `Showing <span class="font-medium text-gray-900">0</span> conditions`;
            } else {
                conditionsCount.innerHTML = `Showing <span class="font-medium text-gray-900">${startIndex}-${endIndex}</span> of <span class="font-medium text-gray-900">${total}</span> condition${total !== 1 ? 's' : ''}`;
            }
        }
    }
    ...
    function checkNoResults() {
        ...
        if (filteredCards.length === 0 && conditionCards.length > 0) {
    ...
    /**
     * Listen for filter changes from conditionFilter.js
    ...
    window.updateConditionPagination = updatePagination;
});
