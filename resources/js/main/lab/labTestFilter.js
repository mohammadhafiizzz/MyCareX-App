document.addEventListener('DOMContentLoaded', () => {
    ...
    function initializeFilters() {
        ...
        // Get all lab test cards
        const labTestCards = document.querySelectorAll('[data-verification]');
        
        console.log('Verification buttons found:', verificationButtons.length);
        console.log('Lab test cards found:', labTestCards.length);
        ...
        /**
         * Filter lab tests based on active filters
         */
        function filterLabTests() {
            ...
            // Show "no results" message if no lab tests match
            updateNoResultsMessage(visibleCount);
            
            // Trigger pagination update
            document.dispatchEvent(new Event('filtersChanged'));
            if (typeof window.updateLabTestPagination === 'function') {
                window.updateLabTestPagination();
            }
        }
        ...
        function updateNoResultsMessage(visibleCount) {
            ...
            const labTestsCountEl = document.getElementById('labtests-count');

            // Update the counter
            if (labTestsCountEl) {
                const countSpan = labTestsCountEl.querySelector('span');
                if (countSpan) {
                    countSpan.textContent = visibleCount;
                }
                // Update plural/singular
                const testText = visibleCount !== 1 ? 's' : '';
                labTestsCountEl.innerHTML = `Showing <span class="font-medium text-gray-900">${visibleCount}</span> test${testText}`;
            }

            // Show/hide no results message
            if (visibleCount === 0 && labTestCards.length > 0) {
                if (noResultsDiv) {
